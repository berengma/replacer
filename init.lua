...
minetest.register_tool( "replacer:replacer",
{
    ...
    tool_capabilities = {
        ...
        groupcaps={
            -- For example:
            fleshy={times={[2]=0.80, [3]=0.40}, maxlevel=0},
            snappy={times={[2]=0.80, [3]=0.40}, maxlevel=0},
...
replacer.replace = function( itemstack, user, pointed_thing, mode )
       ...
       local privs = minetest.get_player_privs(name); -- get information about player privs

   if privs.replace == true then  -- tool only works when replace priv is granted
       ...
       if( not(minetest.setting_getbool("creative_mode") )) then
	    ...
	    and node.name ~= "protector:protect"
     ...
     else minetest.chat_send_player( name, "Error: Sorry you do not have the replace priv"); -- no priv, sorry for that
